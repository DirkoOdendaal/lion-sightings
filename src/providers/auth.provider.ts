...
export class AuthData {
  constructor( public db: AngularFirestore, public database: Database) { }
  ...
  loginUser(email: string, password: string): Promise<any> {
    return this.db.firestore.app.auth().signInWithEmailAndPassword(email, password);
  ...
  signupUser(createUser: User, password: string): Promise<any> {
    return this.db.firestore.app.auth().createUserWithEmailAndPassword(createUser.email, password).then((user) => {
      if (user) {
  ...
  resetPassword(email: string): Promise<any> {
    return this.db.firestore.app.auth().sendPasswordResetEmail(email);
  }
  ...
  logoutUser(): Promise<any> {
    return this.db.firestore.app.auth().signOut();
  }
